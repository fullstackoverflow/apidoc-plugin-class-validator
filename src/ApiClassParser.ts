...
export class ApiClassParser {
	...
	private transferToNewElement({ filePath, classAst, prefix }): IElement[] {
		...
		for (const property of properties) {
			...
			let content_type = this.capitalize(transferType, property);
	...
	private capitalize(text, property) {
		let content_type = "{" + text.charAt(0).toUpperCase() + text.slice(1).toLowerCase();
		if (property.getStructure().decorators) {
			...
			if ((min || max) && allow) {
				content_type += ` {${min.arguments[0] || ""}~${max.arguments[0] || ""}}=${allow.arguments[0].replace("[","").replace("]","")} }`;
			} else if (min || max) {
				content_type += ` {${min.arguments[0] || ""}~${max.arguments[0] || ""}} }`;
			} else if (allow) {
				content_type += `=${allow.arguments[0].replace("[", "").replace("]", "")} }`;
			} else {
				content_type += "}";
			}
		} else {
